...
{
        dbg_print("sched_sleep_on called for thread of process %d\n",curproc->p_pid);
...
{
        dbg_print("sched_cancellable_sleep_on called for thread of process %d\n",curproc->p_pid);
        ktqueue_enqueue(q, curthr);
...
{
        dbg_print("sched_wakeup_on called for thread of process %d\n",curproc->p_pid);
...
{
        dbg_print("sched_broadcast_on called for thread of process %d\n",curproc->p_pid);
        int i = 0;
        for (i = 0; i < q->tq_size; ++i)
        {
                sched_wakeup_on(q);
        }
}
...
{
        dbg_print("sched_cancel called for thread of process %d\n",curproc->p_pid);
        kthr->kt_cancelled = 1;
...
sched_switch(void)
{
        dbg_print("sched_switch entry called for thread of process %d\n",curproc->p_pid);
        ...
        {
            kthread_t *newt = NULL;
            kthread_t *oldt = curthr;
            newt = ktqueue_dequeue(&kt_runq);
            curthr = newt;
            curproc = curthr->kt_proc;
            context_switch(&(oldt->kt_ctx), &(curthr->kt_ctx));
            intr_setipl(ipl);
        }else{
            intr_setipl(IPL_LOW);
            intr_wait();
...
{
        dbg_print("sched_make_runnable called for thread of process %d\n",thr->kt_proc->p_pid);
