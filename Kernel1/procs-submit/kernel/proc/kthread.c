...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
        ...
        KASSERT(NULL != kthr->kt_kstack);
        
        char * pstack = alloc_stack();
        KASSERT(NULL != pstack);
        ...
        context_setup(&(kthr->kt_ctx), func, arg1, arg2, pstack, DEFAULT_STACK_SIZE, p->p_pagedir);
        return kthr;
}
...
{
    KASSERT(NULL != kthr);
    dbg_print("Cancel thread of process %d\n",kthr->kt_proc->p_pid);
    kthr->kt_cancelled = 1;
    if(curthr == kthr){
            kthread_exit(retval);
...
kthread_exit(void *retval)
{
    ...
    dbg_print("thread of process %d exited\n",curthr->kt_proc->p_pid);
    curthr->kt_retval = retval;
