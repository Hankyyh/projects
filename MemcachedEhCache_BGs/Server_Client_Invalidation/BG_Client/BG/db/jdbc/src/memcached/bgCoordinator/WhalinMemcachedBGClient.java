...
package memcached.bgCoordinator;
...
import common.MemcachedEhCacheUtilities;
import common.RdbmsUtilities;
import common.RemoteCacheClient;
...
public class WhalinMemcachedBGClient extends DB implements JdbcDBMemCachedClientConstants {
	...
	//TODO: CHANGED
	private RemoteCacheClient cacheclient = null;
	...
	public boolean init() throws DBException {
		...
		if (initialized) {
			cacheclient = new RemoteCacheClient();
		...
		cacheConnectionPool.setHashingAlg( SockIOPool.NEW_COMPAT_HASH );
...
//				}
	...
	public int viewProfile(int requesterID, int profileOwnerID,
		...
		try {
			...
			payload = common.MemcachedEhCacheUtilities.CacheGet(this.cacheclient, key, this.compressPayload);
			if (payload != null && MemcachedEhCacheUtilities.unMarshallHashMap(result, payload)){
		...
		//serialize the result hashmap and insert it in the cache for future use
		payload = MemcachedEhCacheUtilities.SerializeHashMap(SR);
		try {
			if(!common.MemcachedEhCacheUtilities.CacheSet(this.cacheclient, key, payload, this.compressPayload))
			{
	...
	public int listFriends(int requesterID, int profileOwnerID,
		...
		try {
			//cacheclient = new MemcachedClient(AddrUtil.getAddresses(cache_hostname + ":" + cache_port));
			byte[] payload = common.MemcachedEhCacheUtilities.CacheGet(this.cacheclient, key, this.compressPayload);
			if (payload != null){
				if (verbose) System.out.println("... Cache Hit!");
				if (!MemcachedEhCacheUtilities.unMarshallVectorOfHashMaps(payload,result))
		...
		//serialize the result hashmap and insert it in the cache for future use
			...
			byte[] payload = MemcachedEhCacheUtilities.SerializeVectorOfHashMaps(result);
			try {
				if(!common.MemcachedEhCacheUtilities.CacheSet(this.cacheclient, key, payload, this.compressPayload))
				{
					throw new Exception("Error calling WhalinMemcached set");
		...
		try{
			...
			if(!useTTL)
			{
				...
				if(isInsertImage)
				{
					...
					if(!common.MemcachedEhCacheUtilities.CacheDelete(this.cacheclient, key))
					{
	...
	public int viewTopKResources(int requesterID, int profileOwnerID, int k,
		...
		try {
			//cacheclient = new MemcachedClient(AddrUtil.getAddresses(cache_hostname + ":" + cache_port));
			byte[] payload = common.MemcachedEhCacheUtilities.CacheGet(this.cacheclient, key, this.compressPayload);
			if (payload != null && MemcachedEhCacheUtilities.unMarshallVectorOfHashMaps(payload,result)){
				if (verbose) System.out.println("... Cache Hit!");
				//cacheclient.shutdown();
				return retVal;
			}
